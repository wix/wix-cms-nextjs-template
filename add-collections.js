async function main() {
  ...
  console.log('Adding collections - adding projects collection');
  const resProjects = await fetch(
    `https://manage.wix.com/headless-funnel-nextjs/api/collections/add?refreshToken=${process.env.WIX_REFRESH_TOKEN}&state={"provider":"${provider}"}&clientId=${process.env.NEXT_PUBLIC_WIX_CLIENT_ID}`,
    {
      ...
      body: JSON.stringify({
        payload: {
          collection: {
            displayName: ourProjects.replaceAll('-', ' '),
            fields: [
              {
                key: 'title',
                displayName: 'Title',
  ...
  console.log(
    `Adding collections - added projects collection success: ${JSON.stringify(
      projectsJson
    )}`
  );
  ...
  const resTeam = await fetch(
    `https://manage.wix.com/headless-funnel-nextjs/api/collections/add?refreshToken=${process.env.WIX_REFRESH_TOKEN}&state={"provider":"${provider}"}&clientId=${process.env.NEXT_PUBLIC_WIX_CLIENT_ID}`,
    {
      ...
      body: JSON.stringify({
        payload: {
          collection: {
            displayName: ourTeam.replaceAll('-', ' '),
            fields: [
              {
                key: 'name',
                displayName: 'Name',
  ...
  const resVolunteers = await fetch(
    `https://manage.wix.com/headless-funnel-nextjs/api/collections/add?refreshToken=${process.env.WIX_REFRESH_TOKEN}&state={"provider":"${provider}"}&clientId=${process.env.NEXT_PUBLIC_WIX_CLIENT_ID}`,
    {
      ...
      body: JSON.stringify({
        payload: {
          collection: {
            displayName: volunteers.replaceAll('-', ' '),
            fields: [
              {
                key: 'name',
                displayName: 'Name',
